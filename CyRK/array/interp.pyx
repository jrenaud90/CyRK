...
cdef extern from "interp_common.c":
    Py_ssize_t binary_search_with_guess(double key, double* array, Py_ssize_t length, Py_ssize_t guess) noexcept nogil


cpdef (double, Py_ssize_t) interpj(
        double desired_x, double[:] x_domain, double[:] dependent_values,
    ...
    if provided_j == -2:
        ...
        j = binary_search_with_guess(desired_x, &x_domain[0], len_x, j)
    elif provided_j < -2:
        # Error
...
cdef (double, Py_ssize_t) interpj_ptr(
        double desired_x, double* x_domain, double* dependent_values,
        Py_ssize_t len_x, Py_ssize_t provided_j = -2) noexcept nogil:
    """ Interpolation function for floats. This function will return the index that it found during interpolation.
...
cpdef (double complex, Py_ssize_t) interp_complexj(
        double desired_x, double[:] x_domain, double complex[:] dependent_values,
        Py_ssize_t provided_j = -2) noexcept nogil:
...
cpdef void interp_array(double[:] desired_x_array, double[:] x_domain, double[:] dependent_values,
    ...
    for index in prange(desired_len, nogil=True):
        ...
        j = binary_search_with_guess(desired_x, &x_domain[0], len_x, guess)
