 ...
 */
static double
bnorm(const int n, double* CYRK_RESTRICT a, const int nra, const int ml, const int mu, double* CYRK_RESTRICT w)
{
    int diag, first, last;
    double an = 0.0, sum;
 ...
 */
static double
fnorm(const int n, double* CYRK_RESTRICT a, double* CYRK_RESTRICT w)
{
    double an = 0.0;
    for (int i = 0; i < n; i++)
    {
 ...
 */
static double
vmnorm(const int n, double* CYRK_RESTRICT v, double* CYRK_RESTRICT w)
{
    double vm = 0.0;
    for (int i = 0; i < n; i++)
    {
...
prja(
    ...
    double* wm, int* iwm, c_lsoda_func_t f, c_lsoda_jac_t jac, c_lsoda_common_t* S, void* user_data)
{
    (void)nyh;
    int ier = 0;
    ...
    if ((S->miter != 4) && (S->miter != 5))
    {
        ...
        if (S->miter == 1)
        {
            ...
            // 110
            jac(neq, &S->tn, y, nullptr, nullptr, &wm[2], &S->n, user_data);
            if (*neq == -1) { return; }
            double con = -hl0;
            for (int i = 0; i < lenp; i++)
            {
        ...
        ier = (int)c_dense_lu_factor((size_t)S->n, &wm[2], &iwm[20]);
        if (ier != 0) { S->ierpj = 1; }
        return;
    }
    ...
    if (S->miter == 4)
    {
        ...
        // 410
        jac(neq, &S->tn, y, &ml, &mu, &wm[ml2], &meband, user_data);
        if (*neq == -1) { return; }
        double con = -hl0;
        for (int i = 0; i < lenp; i++)
        {
    ...
    S->pdnorm = bnorm(S->n, &wm[2 + ml], meband, ml, mu, ewt) / fabs(hl0);
    ...
    // 580
    // LU decomposition of P
    ier = (int)c_banded_lu_factor((size_t)S->n, (size_t)ml, (size_t)mu, &wm[2], (size_t)meband, &iwm[20]);
    if (ier != 0) { S->ierpj = 1; }
    return;
 ...
 */
static void
solsy(double* wm, int* iwm, double* x, double* tem, c_lsoda_common_t* S)
{
    ...
    {
        c_dense_lu_solve((size_t)S->n, &wm[2], &iwm[20], x);
        return;
    } else if (S->miter == 3) {
        double phl0 = wm[1];
        double hl0 = S->h * S->el0;
        ...
        c_banded_lu_solve((size_t)S->n, (size_t)ml, (size_t)mu, &wm[2], (size_t)meband, &iwm[20], x);
    }
 ...
 */
static void
stoda_first_call_init(c_lsoda_common_t* S)
{
    S->lmax = S->maxord + 1;
    S->nq = 1;
    S->l = 2;
 ...
 */
static void
stoda_reset(c_lsoda_common_t* S)
{
        // 150
 ...
 */
static void
stoda_adjust_step_size(c_lsoda_common_t* S, double* rh, double* yh, int nyh, const double* sm1)
{
    ...
    c_lsoda_jac_t jac, double pnorm, int* m_out, double* del_out, c_lsoda_common_t* S, void* user_data)
{
    ...
    if (S->ipup > 0) {
        ...
        prja(neq, y, yh, nyh, ewt, acor, savf, wm, iwm, f, jac, S, user_data);
        if (*neq == -1) { return CORRECTOR_ERROR; }
        S->ipup = 0;
        S->rc = 1.0;
        S->nslp = S->nst;
 ...
 */
static void
stoda_get_predicted_values(c_lsoda_common_t* S, double* yh1)
{
...
stoda(
    int* neq, double* y, double* yh, int nyh, double* ewt,
    double* savf, double* acor, double* wm, int* iwm, c_lsoda_func_t f,
    c_lsoda_jac_t jac, c_lsoda_common_t* S, void* user_data)
{
 ...
 * @param S      Pointer to the common structure containing integrator state.
 * @param user_data Opaque pointer handed back to `f` and `jac` on every call.
 */
static void
intdy(const double t, const int k, double* yh, const int nyh, double* dky, int* iflag, c_lsoda_common_t* S)
{
    double hu = S->hu;
    *iflag = 0;
 ...
 */
void c_lsoda(
    c_lsoda_func_t f, int neq, double* y, double* t, double* tout, int itol, double* rtol, double* atol,
    int* itask, int* istate, int* iopt, double* CYRK_RESTRICT rwork, int lrw, int* CYRK_RESTRICT iwork, int liw, c_lsoda_jac_t jac,
    const int jt, c_lsoda_common_t* S, void* user_data)
{
    ...
    if (*istate < 1 || *istate > 3) {
        c_lsoda_mark_error(istate, &S->illin); return;
    }
    if (*itask < 1 || *itask > 5) {
        c_lsoda_mark_error(istate, &S->illin); return;
    }
    ...
    if (*istate == 1)
    {
        ...
        if (neq <= 0) { c_lsoda_mark_error(istate, &S->illin); return; }
        S->n = neq;
        if ((itol < 1) || (itol > 4)) { c_lsoda_mark_error(istate, &S->illin); return; }
        if ((*iopt < 0) || (*iopt > 1)) { c_lsoda_mark_error(istate, &S->illin); return; }
        if (jt == 3 || jt < 1 || jt > 5) { c_lsoda_mark_error(istate, &S->illin); return; }
        S->jtyp = jt;
        if (jt > 2)
        {
            ml = iwork[0];
            mu = iwork[1];
            if ((ml < 0) || (ml >= neq)) { c_lsoda_mark_error(istate, &S->illin); return; }
            if ((mu < 0) || (mu >= neq)) { c_lsoda_mark_error(istate, &S->illin); return; }
        }
        ...
        if (*iopt == 1) {
            // 40
            S->ixpr = iwork[4];
            if ((S->ixpr < 0) || (S->ixpr > 1)) { c_lsoda_mark_error(istate, &S->illin); return; }

            S->mxstep = iwork[5];
            if (S->mxstep < 0) { c_lsoda_mark_error(istate, &S->illin); return; }
            if (S->mxstep == 0) { S->mxstep = mxstp0; }

            S->mxhnil = iwork[6];
            if (S->mxhnil < 0) { c_lsoda_mark_error(istate, &S->illin); return; }
            if (S->mxhnil == 0) { S->mxhnil = mxhnl0; }


            h0 = rwork[4];
            S->mxordn = iwork[7];
            if (S->mxordn < 0)  { c_lsoda_mark_error(istate, &S->illin); return; }
            if (S->mxordn == 0) { S->mxordn = 100; }
            S->mxordn = int_min(S->mxordn, mord[0]);
            S->mxords = iwork[8];
            if (S->mxords < 0) { c_lsoda_mark_error(istate, &S->illin); return; }
            if (S->mxords == 0) S->mxords = 100;
            S->mxords = int_min(S->mxords, mord[1]);
            if ((*tout - *t) * h0 < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }

            hmax = rwork[5];
            if (hmax < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
            S->hmxi = 0.0;
            if (hmax > 0.0) { S->hmxi = 1.0/hmax; }
            S->hmin = rwork[6];
            if (S->hmin < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
        } else {
        ...
        iwork[17] = leniw;
        if (lrw < lenrwc) { c_lsoda_mark_error(istate, &S->illin); return; }
        if (liw < leniwc) { c_lsoda_mark_error(istate, &S->illin); return; }
        S->lewt = len1;
        S->insufr = 0;
        if (lrw < lenrw)
        {
        ...
        for (int i = 0; i < S->n; i++)
        {
            ...
            if (rtoli < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
            if (atoli < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
        }
        // 75
        // go to 100
        ...
        {
            tcrit = rwork[0];
            if ((tcrit - *tout)*(*tout - *t) < 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
        ...
        for (int i = 0; i < S->n; i++)
        {
            if (rwork[S->lewt + i] <= 0.0) { c_lsoda_mark_error(istate, &S->illin); return; }
        ...
        if (h0 == 0.0) {
            ...
            if (tdist < 2.0 * S->uround * w0)  { c_lsoda_mark_error(istate, &S->illin); return; }
        ...
        // block b
        if (neq <= 0) { c_lsoda_mark_error(istate, &S->illin); return; }
        if (neq > S->n) { c_lsoda_mark_error(istate, &S->illin); return; }
        S->n = neq;
        if ((itol < 1) || (itol > 4)) { c_lsoda_mark_error(istate, &S->illin); return; }
        if ((*iopt < 0) || (*iopt > 1)) { c_lsoda_mark_error(istate, &S->illin); return; }
        if (jt == 3 || jt < 1 || jt > 5) { c_lsoda_mark_error(istate, &S->illin); return; }
    ...
    if (*istate != 1)
    {
        ...
        switch (*itask) {
            case 1: {
                ...
                if (iflag != 0)  { c_lsoda_mark_error(istate, &S->illin); return; }
                *t = *tout;
                *istate = 2;
                goto exit;
            }
            ...
            case 3: {
                ...
                if ((tp - *tout) * S->h > 0.0)  { c_lsoda_mark_error(istate, &S->illin); return; }
                if ((S->tn - *tout) * S->h < 0.0) { break; } // go to 250
                *t = S->tn;
                // go to 400
                for (int i = 0; i < S->n; i++) { y[i] = rwork[i + S->lyh]; }
            ...
            case 4: {
                // 230
                tcrit = rwork[0];
                if ((S->tn - tcrit) * S->h > 0.0)  { c_lsoda_mark_error(istate, &S->illin); return; }
                if ((tcrit - *tout) * S->h < 0.0)  { c_lsoda_mark_error(istate, &S->illin); return; }
    ...
    while (1) {
        ...
        if (tolsf > 0.01)
        {
            ...
            {
                rwork[13] = tolsf;
                c_lsoda_mark_error(istate, &S->illin);
                return;
            }
