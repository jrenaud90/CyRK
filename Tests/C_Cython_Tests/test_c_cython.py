...
def test_accuracy_CySolverTester(rk_method):
    ...
    if rk_method == 0:
        assert np.allclose(CySolverAccuracyTestInst.y, real_answer, rtol=1.0e-3, atol=1.0e-4)
    elif rk_method == 1:
        assert np.allclose(CySolverAccuracyTestInst.y, real_answer, rtol=1.0e-4, atol=1.0e-5)
    else:
        assert np.allclose(CySolverAccuracyTestInst.y, real_answer, rtol=1.0e-5, atol=1.0e-6)
