...
from CyRK import nbsolve2_ivp as nbsolve_ivp
...
def diffeq_args(dy, t, y, args):
    dy[0] = (1. - args[0] * y[1]) * y[0]
    dy[1] = (args[1] * y[0] - 1.) * y[1]

def diffeq_extra(dy, t, y, args):
    extra_1 = (1. - 0.01 * y[1])
    extra_2 = (0.02 * y[0] - 1.)
    dy[0] = extra_1 * y[0]
    dy[1] = extra_2 * y[1]
...
def diffeq_args_extra(dy, t, y, args):
    extra_1 = (1. - args[0] * y[1])
    extra_2 = (args[1] * y[0] - 1.)
    dy[0] = extra_1 * y[0]
    dy[1] = extra_2 * y[1]
    dy[2] = extra_1
    dy[3] = extra_2
...
args = np.asarray((0.01, 0.02), dtype=np.float64, order='C')
...
def test_nbsolve2_ivp_test():
    """Check that the builtin test function for the `nbsolve2_ivp` integrator is working"""
...
def test_pysolve_ivp(use_args, 
    ...
    if use_different_tols:
        ...
        rtol /= 100.0
        atol /= 100.0
        if rtol_arr is not None:
            rtol_arr /= 100.0
        if atol_arr is not None:
            atol_arr /= 100.0
    ...
    diffeq_addr = nb_diffeq_addr(diffeq_to_use)
    result = None
    reuses = 0
    while reuses < 4:
        result = \
            nbsolve_ivp(diffeq_addr, time_span_touse, initial_conds,
                        method=integration_method,
                        args=args_touse, rtol=rtol, atol=atol, rtols=rtol_arr, atols=atol_arr,
                        num_extra=num_extra, first_step=first_step, max_step=max_step)

        assert isinstance(result, NbCySolverResult)
...
def test_pysolve_ivp_accuracy(integration_method, t_eval_end, test_dense_output, backward_integrate):
    ...
    @njit
    def diffeq_accuracy(dy, t, y, args):
    ...
    diffeq_addr = nb_diffeq_addr(diffeq_accuracy)
    result = \
        nbsolve_ivp(diffeq_addr, time_span_, y0, method=integration_method, t_eval=t_eval,
                    dense_output=test_dense_output,
...
def test_pysolve_ivp_readonly(integration_method):
    ...
    diffeq_addr = nb_diffeq_addr(diffeq_accuracy)
    result = \
        nbsolve_ivp(diffeq_addr, time_span_, y0, method=integration_method,
                    rtol=1.0e-8, atol=1.0e-9)
